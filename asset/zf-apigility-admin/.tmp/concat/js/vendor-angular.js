...
(function () {
  ...
  angular.module('angular-flash.service', []).provider('flash', function () {
    var _self = this;
      ...
      return {
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
          ...
          input.on('keydown', function (e) {
            ...
            var key = e.keyCode, isModifier = e.shiftKey || e.altKey || e.ctrlKey || e.metaKey;
            if (isModifier || hotkeys.indexOf(key) === -1) {
              return;
            }
            if (key === KEYS.enter && scope.options.addOnEnter || key === KEYS.comma && scope.options.addOnComma || key === KEYS.space && scope.options.addOnSpace) {
              if (scope.tryAdd()) {
                scope.$apply();
              }
              e.preventDefault();
        ...
        return {
          load: function (directive, scope, attrs, options) {
            ...
            angular.forEach(options, function (value, key) {
              var interpolatedValue = attrs[key] && $interpolate(attrs[key])(scope.$parent), converter = converters[value[0]], getDefault = function (key) {
  ...
  return {
    ...
    link: function ($scope, element, attrs) {
      ...
      attrs.$observe('knobLabel', function (val) {
        $scope.knobLabel = angular.isDefined(val) ? val : '\xa0';
      });
